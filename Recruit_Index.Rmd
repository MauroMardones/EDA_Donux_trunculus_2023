...
denspobtot2 <- denspobtot %>% 
  ...
  summarise(Btotal=sum(TCSW)) %>% 
  mutate(fpn = MCSWsub/CSW,
         NtotalCSW  = Nmedida * fpn, 
         Ndaños = Ndañossub * fps, 
...
```{r warning=FALSE}
duda <- ggplot(D15duda,aes(MES, Densidad))+
  geom_point()+
...
D15_17_20
```
Ambos graficos (2017-2020 y 2021.2024)


```{r warning=FALSE}
ggarrange(D15_17_20, plotD15, common.legend=TRUE, legend="bottom")

combined <- D15_17_20 + plotD15 & theme(legend.position = "bottom")
...
Ahora construyo un indice de densidad, por que es mas *ad-hoc* para los modelos de stock assessment (Caddy, 2004). 
