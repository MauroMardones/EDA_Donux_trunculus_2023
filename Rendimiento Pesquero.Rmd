...
#write.csv(rend1723, "CPUE_1725.csv")
...
rendgru <- ggplot(grurend1723 , 
    ...
    geom_smooth(method = "lm", se = FALSE) +  # Cambia "lm" por "loess"
    theme_few() + 
    facet_grid(~ ANO) +
    theme(
...
# otro plot con los valore de la regresion

lm_results <- grurend1723 %>%
  group_by(ANO) %>%
  do({
    mod <- lm(MEAN ~ as.numeric(MES), data = .)
    tidy_mod <- tidy(mod)
...
labels_df <- lm_results %>%
  mutate(label = ifelse(pval < 0.05,
                        paste0("Pend: ", round(slope, 3),
                               "\nP<0.05"),
                        paste0("Pend: ", round(slope, 3),
                               "\nP=", signif(pval, 2)))) %>%
  left_join(grurend1723 %>% group_by(ANO) %>% 
              summarise(x = median(as.numeric(MES), na.rm = TRUE),
                        y = max(MEAN, na.rm = TRUE) + 1),
            by = "ANO")

rendgru <- ggplot(grurend1723 , 
                  aes(x = as.numeric(MES), y = MEAN)) +
  geom_point(alpha = .7) +
  ...
  scale_x_continuous(breaks = seq(min(as.numeric(grurend1723$MES), na.rm = TRUE),
                                  max(as.numeric(grurend1723$MES), na.rm = TRUE),
                                  by = 2)) +
  geom_hline(yintercept = 7.6, col = "black") +
  geom_hline(yintercept = 3.5, col = 2) +
  ylab("CPUE (Kg/Hr)") +
...
Tendencia de los últimos 8 años
```{r}
grurend1723 <- rend1723 %>%
