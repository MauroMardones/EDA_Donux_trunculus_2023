...
masadi <- ggplot() +
  geom_sf(data = costa1, fill="white") +
  geom_sf(data = portug1, fill="white") +
  geom_sf(data = resepe1, aes(fill=Zona),
          alpha=0.1) +
  # geom_sf(data = fisicomar1, alpha=0.1,
  #         linetype=5) +
  scale_fill_viridis_d(option="H",
...
# Descargar batimetría (resolución 1 minuto aprox.)
bathy <- marmap::getNOAA.bathy(lon1 = -7.6, lon2 = -6.1, 
                               lat1 = 36.6, lat2 = 37.4, 
                               resolution = 1)
...
```{r warning=FALSE}
breaks_vals <- seq(-1000, 0, by = 10)  # todos los breaks cada 10 m

maspu <- ggplot() +
  # Batimetría en tonos celestes
  geom_contour_filled(
    data = bathy_df, 
    aes(x = x, y = y, z = z),
    breaks = breaks_vals
  ...
  scale_fill_manual(
    values = colorRampPalette(c("#00264d", "#005b96", "#66b2ff", "#cce6ff"))(length(breaks_vals)-1),
    name = "Profundidad (m)"
  ) +
  # Líneas de contorno segmentadas
  geom_contour(
    data = bathy_df,
    aes(x = x, y = y, z = z),
    breaks = breaks_vals,
  ...
  # Capas vectoriales
  geom_sf(data = costa1, fill = "white") +
  geom_sf(data = zonapro1, fill = NA, color = "red") +  # línea roja, interior transparente
  geom_sf(data = coord) +
  # Etiquetas de puntos
  ggrepel::geom_label_repel(
    data = coord,
  ...
  coord_sf(xlim = c(-7.6, -6.1), ylim = c(36.6, 37.4)) +
  xlab(expression(paste(Longitude^o,~'O'))) +
