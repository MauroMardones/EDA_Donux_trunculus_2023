...
# Ejemplo de datos de entrada con coordenadas en formato "40° 42.767' N"
data <- data.frame(coordenadas = c("40° 42.767' N", "34° 12.345' S", "28° 55.123' N"))

# Función para convertir coordenadas DMS a decimales
convertir_a_decimal <- function(coordenada) {
  partes <- unlist(strsplit(coordenada, "[° ' N]"))
  grados <- as.numeric(partes[1])
  minutos <- as.numeric(partes[2])
  decimales <- as.numeric(paste0("0.", partes[3]))
  
  # Determinar si la coordenada está en el hemisferio sur (negativo)
  if (grepl("S", coordenada)) {
    signo <- -1
  } else {
    signo <- 1
  }
  ...
  decimal <- signo * (grados + minutos / 60 + decimales / 3600)
  return(decimal)
}

# Aplicar la función a la columna de coordenadas
data$coordenadas_decimal <- sapply(data$coordenadas, convertir_a_decimal)
