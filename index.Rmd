...
landpop
```
Ahora por la cantidad de individuos bajo los 15 mm. Estoy usando los datos desde el 2017, pero previo al 2020, no se puede asignar un area a los datos.
...
%>%
  ...
  ungroup() %>%
  mutate(num_individuos_m2 = num_individuos / (m_track)) %>% 
  mutate(logd =log(num_individuos_m2 )) %>% 
  arrange(ANO, MES)
D15n
```


Hago la table de resultados
...
# Aplicar pivot_wider
datos_pivote <- pivot_wider(D15n, names_from = Sampling.point.x, values_from = c(num_individuos_m2))

# Ordenar el dataframe resultante por una columna específica
datos_pivote_ordenado <- datos_pivote %>% arrange(ANO, MES)

# Imprimir los resultados
write_csv(datos_pivote_ordenado, "D15")
```
...
```{r}
DN15plot <- ggplot(D15n %>% 
                  filter(Sampling.point.x== c(2,4,6)), 
               aes(MES,logd))+
    geom_col(position = "dodge")+
    theme_few()+ 
    facet_grid(Sampling.point.x~ANO)+
    #scale_y_discrete(breaks = seq(from = 1, to = 13, by = 1))+
    theme(axis.text.x = element_text(angle = 90,
                                     hjust = 1,
                                     vjust= 0.5,
...
lmarino <- st_read('~/IEO/DATA/Shapefiles_Andalucia/13_36_LimiteMarino.shp')

zzpp_resolucion_2023_25830
```
