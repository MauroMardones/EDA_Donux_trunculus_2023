...
```{r}
# Abrir el archivo
nc <- nc_open("~/IEO/FEMP_04/Ambientales_Data/SST_ERA5/29a7f9d3886cf06dc79bacbdfda33c76.nc")

# Ver qué variables tiene
names(nc$var)
...
n_time <- dim(sst_data)[3]
dates <- seq.Date(start_date, by = "month", length.out = n_time)


# promedio global por capa temporal y pasar a °C
sst_mean <- apply(sst_data, 3, mean, na.rm = TRUE) - 273.15

start_date <- as.Date("2000-01-01")
n_time <- dim(sst_data)[3]
dates <- seq.Date(start_date, by = "month", length.out = n_time)

df <- data.frame(date = dates, sst = sst_mean) %>%
...
SST_Cadiz <- ggplot(df %>% 
         filter(year>2016), aes(x = month, y = sst_month, color = factor(year), group = year)) +
  geom_line() +
  geom_point() +
  geom_rect(
  ...
  labs(x = "Mes", y = "SST promedio (°C)", color = "") +
  theme_bw()
SST_Cadiz
