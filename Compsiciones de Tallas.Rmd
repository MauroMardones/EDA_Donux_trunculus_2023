...
size2017 <- read.csv2(here("Data", 
                      ...
                      dec=".") %>% 
  dplyr::select(c(22, 3, 4, 12, 19, 20, 21, 2)) %>% 
   mutate(
    ID_codificado_muestreo=NA
    )%>% 
  rename(sizeE=SizeE,
...
sizeall3 <- sizeall2 %>%
  mutate(MES = factor(MES, 
                      levels = rev(levels(MES))))  # invierte el orden


nall <- ggplot(sizeall3 %>% 
                 drop_na(sizeE, MES), 
               aes(x = sizeE, 
                   y = MES,
                   fill = as.factor(rastro))) +
  # Franja gris entre mayo y julio
  ...
  scale_fill_viridis_d(option = "B", name = "Gear") +
  theme_few() +
...
pmea <- ggplot(sizemean %>% 
    ...
    theme_few() + 
    geom_smooth(method = "loess") +  # Método "lm" para suavizado lineal
...
pmecom <- ggplot(sizemean %>% 
  ...
  geom_smooth(method = "lm") +
  geom_rect(
    aes(xmin = 4.5, xmax = 7.5, ymin = -Inf, ymax = Inf),  # meses 5–7
...
```{r, fig.align="center", warning=FALSE}
# Calcular medias por año para cada tipo de rastro
medias_poblacional <- sizemean %>% 
  filter(Sampling.point %in% c(2, 4, 6, "M"),
         rastro != "COMERCIAL") %>% 
  drop_na(avg, Sampling.point) %>%
  group_by(ANO) %>%
  summarise(media_ano = round(mean(avg, na.rm = TRUE), 2))

medias_comercial <- sizemean %>% 
  filter(Sampling.point %in% c(2, 4, 6, "M"),
         rastro != "POBLACIONAL") %>% 
  drop_na(avg, Sampling.point) %>%
  group_by(ANO) %>%
...
# Gráfico POBLACIONAL con media por año
pmecom <- ggplot(sizemean %>% 
                   filter(Sampling.point %in% c(2, 4, 6, "M"),
                          rastro != "COMERCIAL") %>% 
                   drop_na(avg, Sampling.point), 
                 aes(as.numeric(MES), avg)) +
  geom_point(aes(x = as.factor(MES)),
             show.legend = TRUE,
             alpha = 0.7) + 
  geom_smooth(method = "lm") +
  geom_rect(
    aes(xmin = 4.5, xmax = 7.5, ymin = -Inf, ymax = Inf),
    fill = "red", alpha = 0.01, inherit.aes = FALSE
  ) +
  geom_text(data = medias_poblacional,
            aes(x = 6, y = 10, label = paste0(media_ano, " mm")),
            inherit.aes = FALSE,
            size = 3.5) +
  theme_few() + 
  facet_grid( ~ ANO) +
...
# Gráfico COMERCIAL con media por año
pmepob <- ggplot(sizemean %>% 
                   filter(Sampling.point %in% c(2, 4, 6, "M"),
                          rastro != "POBLACIONAL") %>% 
                   drop_na(avg, Sampling.point), 
  ...
  geom_smooth(method = "lm", col = "red") +
  geom_text(data = medias_comercial,
            aes(x = 6, y = 10, label = paste0(media_ano, " mm")),
            inherit.aes = FALSE,
            size = 3.5) +
  theme_few() + 
...
ggarrange(pmecom, pmepob, ncol = 1)
